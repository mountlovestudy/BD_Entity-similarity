...
class kernels:
    def __init__(self,alpha,gammas,theta2):
        self.alpha=np.exp(alpha)
        self.gammas=np.exp(gammas)
        #self.theta1=np.exp(theta1)
        self.theta2=np.exp(theta2)
        self.dim=gammas.size
        self.nparams=self.dim+2
        
    def set_params(self,params):
        assert params.size==self.nparams
        self.alpha=np.exp(params).copy().flatten()[0]
        self.gammas=np.exp(params).copy().flatten()[1:self.dim+1]
        #self.theta1=np.exp(params).copy().flatten()[self.dim+1]
        self.theta2=np.exp(params).copy().flatten()[self.dim+1]
        
        
    def get_params(self):
        #print np.hstack((self.alpha,self.gammas,self.theta1,self.theta2))
        return np.log(np.hstack((self.alpha,self.gammas,self.theta2)))
    ...
    def __call__(self,x1,x2):
        ...
        diff=self.alpha*np.exp(-np.sum(np.square(diff)*self.gammas,-1)/2)+np.eye(N1,N2)/self.theta2
    ...
    def gradients(self,x1):
        ...
        #grads.append(self.theta1*np.ones([N1,N1]))
        grads.append(-delta)
        return grads
